            ...
            <template v-slot:item.activity="{ item }">
                <v-menu>
                    <template v-slot:activator="{ props }">
                        <label v-bind="props" class="text-primary fw-bolder"><b>{{ (item.activity) }}</b></label>
                    </template>
                    <v-list border rounded="lg" density="compact" class="py-0">
                        <template v-if="role == 1">
                            <v-list-item @click="$emit('view', item.riskId)">
...
export default {
    ...
    emits: ['submit', 'validate', 'view', 'edit', 'removed'],
    data() {
        return {
            search: '',
            selectedYear: null,
